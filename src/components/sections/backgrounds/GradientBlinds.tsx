...
/**
 * GradientBlinds — vertical light shafts with a spotlight that follows the
 * pointer. Used behind the first tab of "Find the right solution".
 *
 * From react-bits, unchanged in behaviour. Two adaptations for this codebase:
 * the component's stylesheet import is dropped (its one rule lives in
 * vxn-home-ae.css with the rest of the section), and `className` no longer
 * stringifies as "undefined" when it is not passed.
 */
...
const GradientBlinds: React.FC<GradientBlindsProps> = ({
  ...
  /* Read inside the frame loop, so flipping it must not re-create the context. */
...
`;
    ...
    /* Bound to the whole section rather than the canvas: the background is
       pointer-events:none, and the pointer is normally over the copy. */
    const surface = (container.closest('.vxs') as HTMLElement | null) ?? container;
    ...
    const loop = (t: number) => {
      ...
      if (mouseDampening > 0) {
        ...
        cur[0] += (targetPos[0] - cur[0]) * factor;
        cur[1] += (targetPos[1] - cur[1]) * factor;
      } else {
        lastTimeRef.current = t;
      }
    ...
    // Every prop below is fixed for the life of this instance.
