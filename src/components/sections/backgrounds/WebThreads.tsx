...
/**
 * WebThreads — a fan of drawn threads that bends toward the pointer.
 *
 * From react-bits, behaviour unchanged. Three adaptations for this codebase:
 * the component's stylesheet import is dropped (its one rule lives in
 * vxn-home-ae.css with the rest of the section), the frame loop does not start
 * when the visitor has asked for reduced motion, and a failed WebGL2 context
 * leaves the card empty rather than throwing.
 *
 * It already pauses itself when scrolled out of view or when the tab is hidden.
 */
...
const WebThreads: React.FC<WebThreadsProps> = ({
  ...
  /* Every prop is fixed for the life of this instance (see AboutBento, which
     hoists them to module constants), so the context is built once. */
  const props = useRef({
    ...
    grainIntensity, mouseInteraction, mouseStrength, backgroundColor, lightMode,
  });
  ...
  useEffect(() => {
    ...
    try {
      ...
      return; // no WebGL2: the card stays empty
    }
    ...
    const program = new Program(gl, {
      ...
      uniforms: {
        ...
        uThreadCount: { value: Math.round(p.threadCount) },
        uFrequency: { value: p.frequency },
        uSpread: { value: p.spread },
        uTaper: { value: p.taper },
        uPosition: { value: p.position },
        uFanMode: { value: FAN_MODE[p.fanMode] ?? 0 },
        uGlow: { value: p.glow },
        uFalloff: { value: p.falloff },
        uThickness: { value: p.thickness },
        uBrightness: { value: p.brightness },
        uOpacity: { value: p.opacity },
        uMirror: { value: p.mirror ? 1.0 : 0.0 },
        uShimmer: { value: p.shimmer ? 1.0 : 0.0 },
        uGrain: { value: p.grain ? 1.0 : 0.0 },
        ...
        uLightMode: { value: p.lightMode },
        uMouse: { value: new Float32Array([0.5, 0.5]) },
        uMouseStrength: { value: p.mouseStrength },
        uEnableMouse: { value: p.mouseInteraction ? 1.0 : 0.0 },
